pipeline {
    ...
    stages {
        ...
        stage('docker build local image') {
            steps {
                sh 'docker build . -t marcorezkallah/nti-cicd-project:1.$BUILD_NUMBER'
            }
        }

        stage('docker push local image') {
            steps {
                sh 'docker push marcorezkallah/nti-cicd-project:1.$BUILD_NUMBER'
            }
        }

        stage('docker tag latest') {
            steps {
                sh 'docker image tag marcorezkallah/nti-cicd-project:1.$BUILD_NUMBER marcorezkallah/nti-cicd-project:latest'
            }
        }
